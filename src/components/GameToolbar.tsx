  ...
  return (
    <div className="flex items-center justify-center gap-8 py-4">
      ...
      <button
        ...
        className={`game-toolbar-btn ${disabled || !canUndo ? "opacity-40" : ""}`}
        aria-label="Hoàn tác"
      >
        <svg
          className="w-7 h-7"
      ...
      <button
        onClick={onErase}
        disabled={disabled}
        className={`game-toolbar-btn ${disabled ? "opacity-40" : ""}`}
        aria-label="Xóa"
      >
        <svg
          className="w-7 h-7"
      ...
      <button
        ...
        disabled={disabled}
        className={`game-toolbar-btn ${isNotesMode ? "game-toolbar-btn-active" : ""} ${disabled ? "opacity-40" : ""}`}
      ...
      >
        <div className="relative">
          ...
          {!isNotesMode && (
            <span className="absolute -bottom-1 -right-1 bg-gray-400 text-white text-[8px] px-1 rounded">
              OFF
            </span>
          )}
        </div>
      ...
      <button
        ...
        className={`game-toolbar-btn relative ${disabled || hintsRemaining === 0 ? "opacity-40" : ""}`}
